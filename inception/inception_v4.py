...
flags.DEFINE_string(
    'precision', 'float32',
    help=('Precision to use; one of: {bfloat16, float32}; Will always use float32 for gpu'))
...
# fine-tuning
flags.DEFINE_string(
    'checkpoint_path', None, 'The checkpoint or the path to retore for fine-tuning.')
flags.DEFINE_string(
    'checkpoint_exclude_scopes', None, 'The layers to exclude from fine-tuning.')
...
class InputPipeline(object):
  ...
  def input_fn(self, params):
    ...
    if 'batch_size' in params:
      ...
      batch_size = (FLAGS.train_batch_size if self.is_training
                    else FLAGS.eval_batch_size)

    if FLAGS.use_data == 'real':
      file_pattern = os.path.join(
          self.data_dir, 'drinks_train_*' if self.is_training else 'drinks_validation_*')
...
def inception_model_fn(features, labels, mode, params):
  ...
  loss = losses + FLAGS.weight_decay * tf.add_n(l2_loss)
...
def main(unused_argv):
  ...
  batch_axis = 0
  if FLAGS.transpose_enabled and FLAGS.use_tpu:
  ...
  if FLAGS.eval_total_size > 0:
    ...
    eval_size = FLAGS.num_eval_images
  ...
  # Initialize weights from latest ckpt to fine-tune
  ...
  # Excluding the layers to fine-tune
  exclusions = []
  ...
  ws = tf.estimator.WarmStartSettings(ckpt_to_initialize_from=checkpoint_path,
                                      vars_to_warm_start='(?!.*{}).*'.format(exclusions))

  if FLAGS.use_tpu:
    run_config = tf.contrib.tpu.RunConfig(
        cluster=tpu_cluster_resolver,
        model_dir=FLAGS.model_dir,
        save_checkpoints_steps=(FLAGS.save_checkpoints_steps or FLAGS.iterations),
